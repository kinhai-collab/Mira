...
export default function Dashboard() {
	...
	// Render custom calendar with full functionality
	const renderCalendar = () => {
		...
		return (
			<div className="calendar-container absolute top-full right-0 sm:right-auto sm:left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl p-3 sm:p-4 z-50 w-[calc(100vw-2rem)] sm:w-72 max-w-sm">
				{/* Calendar Header with Navigation */}
				<div className="mb-3 sm:mb-4 flex items-center justify-between">
					<button
						onClick={() => navigateMonth('prev')}
						className="p-2 sm:p-1 hover:bg-gray-100 active:bg-gray-200 rounded transition min-h-[44px] min-w-[44px] touch-manipulation flex items-center justify-center"
					...
					<h3 className="text-sm sm:text-base font-bold text-gray-800 text-center flex-1 px-2">
				...
				<div className="grid grid-cols-7 gap-0.5 sm:gap-1 mb-2">
					{days.map((day) => (
						<div key={day} className="text-[10px] sm:text-xs font-bold text-gray-800 text-center py-1">
							{day}
						</div>
					))}
				</div>
				...
				<div className="grid grid-cols-7 gap-0.5 sm:gap-1">
					{calendarDays.map((item, index) => {
						...
						const isSelected = dateStr === selectedDate && item.type === 'current';
						const events = getEventsForDate(item.year, item.month, item.day);
						...
						if (item.type === 'prev') {
							return (
								<button
									...
									className="text-[10px] sm:text-xs text-gray-300 text-center py-2 sm:py-2.5 rounded hover:bg-gray-50 active:bg-gray-100 transition min-h-[36px] sm:min-h-[40px] touch-manipulation"
								>
									{item.day}
								</button>
							);
						...
						return (
							<button
								key={`current-${index}`}
								onClick={() => handleDateSelect(item.day, item.month, item.year)}
								className={`text-[10px] sm:text-xs text-center py-2 sm:py-2.5 rounded transition relative min-h-[36px] sm:min-h-[40px] touch-manipulation ${
									isSelected
								...
								title={events.length > 0 ? events.join(', ') : ''}
							>
								{item.day}
								{events.length > 0 && (
									<span className="absolute bottom-0.5 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-blue-500 rounded-full"></span>
								)}
							</button>
						);
					})}
				...
				{/* Events Display */}
				{(() => {
					const selectedEvents = getEventsForDate(selectedYear, selectedMonth, selectedDay);
					if (selectedEvents.length > 0) {
						return (
							<div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-200">
								<p className="text-[10px] sm:text-xs font-semibold text-gray-700 mb-2">Events:</p>
								<div className="space-y-1">
									{selectedEvents.map((event, idx) => (
										<div key={idx} className="text-[10px] sm:text-xs text-gray-600 bg-gray-50 px-2 py-1 rounded">
											{event}
										</div>
									))}
								</div>
	...
	return (
		<div className="flex flex-col md:flex-row h-screen bg-[#F8F8FB] text-gray-800 overflow-hidden">
			{/* Main Content */}
			<main className={`flex-1 px-4 sm:px-8 md:px-12 py-8 md:py-10 ${viewMode === "emails" ? "overflow-y-auto" : "overflow-y-auto"}`}>
				{viewMode === "overview" ? (
					<>
						{/* Header */}
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
					...
					<>
						{/* Emails View - Matching Image Exactly */}
						...
						{/* Summary Bar */}
						<div className="bg-white border border-gray-200 rounded-lg px-3 sm:px-4 md:px-6 py-3 sm:py-4 mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
							<div className="text-gray-700 text-sm sm:text-base font-medium">12 Important Emails</div>
							<div className="flex flex-wrap items-center gap-2 sm:gap-3">
								<span className="bg-red-500 text-white text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 rounded-full font-medium">
									High: 3
								</span>
								<span className="bg-yellow-500 text-white text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 rounded-full font-medium">
									Medium: 5
								</span>
								<span className="bg-green-500 text-white text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 rounded-full font-medium">
									Low: 4
								</span>
								<div className="flex items-center gap-1.5 sm:gap-2 text-gray-500 text-xs sm:text-sm sm:ml-4 relative calendar-container">
									<div 
										onClick={() => setShowCalendar(!showCalendar)}
										className="flex items-center gap-1.5 sm:gap-2 cursor-pointer hover:opacity-80 active:opacity-60 transition min-h-[44px] touch-manipulation px-1"
									>
										<Image
											src="/Icons/Property 1=Calendar.svg"
											alt="Calendar"
						...
						<div className="bg-white border border-gray-200 rounded-lg overflow-hidden flex flex-col">
							{/* Tabs */}
							<div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b border-gray-200 flex-shrink-0 overflow-x-auto">
								<div className="flex gap-4 sm:gap-6 min-w-max">
									{(["All", "High", "Medium", "Low"] as PriorityFilter[]).map((tab) => (
										<button
											key={tab}
											onClick={() => setPriorityFilter(tab)}
											className={`text-xs sm:text-sm font-medium transition pb-1 min-h-[44px] touch-manipulation px-1 ${
												priorityFilter === tab
							...
							<div className="divide-y divide-gray-100 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 400px)', scrollbarWidth: 'thin' }}>
								{filteredEmails.map((email) => {
									...
									return (
										<div key={email.id} className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 hover:bg-gray-50 active:bg-gray-100 transition flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
											{/* Priority Bar and Content Row */}
											<div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
												{/* Priority Bar */}
												<div className={`w-0.5 sm:w-1 h-12 sm:h-14 ${priorityColor} rounded-full flex-shrink-0`}></div>
												
												{/* Avatar */}
												<div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full ${avatarBg} flex items-center justify-center font-semibold text-xs sm:text-sm flex-shrink-0`}>
													{initials}
												</div>
												
												{/* Email Content */}
												<div className="flex-1 min-w-0">
													<div className="font-semibold text-sm sm:text-base text-gray-800 mb-0.5 sm:mb-1 truncate">{email.sender}</div>
													<div className="text-xs sm:text-sm text-gray-600 truncate">{email.subject}</div>
												</div>
											</div>
											
											{/* View Button and Time - Responsive Layout */}
											<div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 flex-shrink-0 sm:w-auto w-full sm:min-w-[180px]">
												<button 
													onClick={() => handleEmailView(email)}
													className="px-3 sm:px-4 py-2 sm:py-1.5 text-xs font-medium border border-gray-300 rounded-full hover:bg-gray-50 active:bg-gray-100 transition whitespace-nowrap flex-shrink-0 min-h-[44px] touch-manipulation"
												>
													View
												</button>
												<div className="flex items-center gap-1 sm:gap-1.5">
													<Icon name="Clock" size={10} className="sm:w-3 sm:h-3 text-gray-500" />
													<span className="text-[10px] sm:text-xs text-gray-500 whitespace-nowrap">{formatTimeAgo(email.timeAgo)}</span>
												</div>
											</div>
										</div>
									);
